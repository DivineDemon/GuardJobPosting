  ...
  connection.query(
    ...
    (err, rows, fields) => {
      if (!err) {
        res
          .status(201)
          .json({ success: true, message: "Data Inserted Successfully!", data: rows });
      } else {
        res.status(500);
        throw new Error(err);
      }
...
const deleteShift = (req, res) => {
  ...
  connection.query(`DELETE FROM shift WHERE shiftID=${id}`, (err, rows) => {
    if (!err) {
      res.status(201).json({ success: true, message: "Shift Deleted Successfully!" });
    } else {
      res.status(500);
      throw new Error(err);
    }
...
const updateShift = (req, res) => {
  ...
  connection.query(
    ...
    (err, rows) => {
      if (!err) {
        res.status(201).json({ success: true, message: "Shift Updated Successfully!" });
      } else {
        res.status(500);
        throw new Error(err);
      }
